...
- **Use callouts for important notes.** Use the `<Callout type="warning" title="Title">` (or `type="info"`, `type="error"`) component for gotchas, tips, and prerequisites. Example: **Save before you deploy** — changes after the last save won't appear in the deployed Flow.
- **Standardize placeholders.** In code examples, use angle brackets for values the user must replace: `<YOUR_API_KEY>`, `<PROJECT_ID>`, `<FLOW_ID>`, `<PROJECT_ENDPOINT>`. Add a short "Prerequisites" or "Replace these values" line before the snippet.
- **Terminology.** Use **Flow** and **Node** (capitalized) as the product terms. Avoid "workflow" when you mean Flow unless the UI uses it. Keep terms consistent with the [Glossary](/docs/glossary).
...
4. **Typo**: A small error in the text, such as a misspelling or grammatical mistake.
