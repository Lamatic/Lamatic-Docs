...
# Vector Action Node Documentation
...
2. **Embedding & Metadata Support**: Attach vector embeddings and metadata for robust search and filtering.
3. **Conditional Filtering**: Apply advanced JSON filters with logical operators (`And`, `Or`, etc.).
4. **Primary Key Matching**: Use custom primary keys to uniquely identify or filter/delete entries.
5. **Duplicate Handling**: Choose to `overwrite` or `skip` on indexing conflicts.
