...
<NodeOverview slug="keyword-search-node" type="data" />
...
<NodeTypeInfo 
  ...
  eventTrigger={false}
  action={true}
  description="This node is an Action node that performs keyword-based search operations to find relevant content in datasets."
/>
...
1. **Customizability**: Dynamic query capabilities ensure search inputs can adapt to varying flow and datasets.
...
## Output
- `searchResults`: An array of objects representing the results of the keyword search.
