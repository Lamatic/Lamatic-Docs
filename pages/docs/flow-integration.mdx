...
If your application, API, or webhook receiver is behind a firewall, review [IP Allowlisting](/docs/whitelist-ip) before going live.
...
Replace `<FLOW_ID>`, `<PROJECT_ID>`, and the endpoint with the values from the Connect modal. Get your API key from **Studio → Settings → API Keys** or the **Get API Key** link in the modal. See [API Overview](/docs/api-integration) and [API Keys](/docs/api-integration/api-key) for more.
...
- For more API details, see [API Overview](/docs/api-integration) and [Integration Guide](/docs/sdk/integration-guide).
