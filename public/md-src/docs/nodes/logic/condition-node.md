...
<NodeTypeInfo 
  ...
  eventTrigger={false}
  action={true}
  description="This node is an Action node that evaluates conditions and determines workflow execution paths based on logic and variables."
/>
...
<details>
  ...
  5. **Multiple Condition Execution:** Enable flexible multi-value matching with the "Allow Multiple Condition Execution" option for advanced workflow routing.
  6. **Enhanced Output Support:** Supports both string and array outputs for condition evaluation, maintaining backward compatibility with existing flows.
</details>
...
5. **Enable Multiple Condition Execution (Optional)**:

   - Toggle the **Allow Multiple Condition Execution** option to enable flexible multi-value matching.
   - When enabled, the node can execute multiple condition paths simultaneously if multiple conditions evaluate to `true`, allowing for more flexible workflow routing.
   - This feature is particularly useful when you need to handle scenarios where input values match multiple conditions at once.

6. **Define Outcome Paths**:
...
## Output 
- `condition`: The evaluated outcome or state determined by the condition node. Can be either:
  - **String**: A single condition label when one condition matches (default behavior, maintains backward compatibility).
  - **Array**: Multiple condition labels when "Allow Multiple Condition Execution" is enabled and multiple conditions evaluate to `true`.

### Example Output

**Single Condition (String Output):**
```json
{
    "condition": "Else"
}
...
**Multiple Conditions (Array Output):**
```json
{
