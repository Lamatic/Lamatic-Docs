...
Flows are the core building blocks of Lamatic. A Flow connects multiple **Nodes** into a sequence so you can process data, call tools/models, and return a final output.

## What you’ll learn

- What a Flow is and when to use one
- The key parts of a Flow (Trigger → Actions → Response)
- How to create, test, share, and deploy a Flow

## Prerequisites

- A Lamatic Project in Studio
- Basic understanding of [Nodes](/docs/nodes)
...
- Trigger work from an event or request (API, widget, Slack, schedules, database sync)
- Chain multiple steps (retrieve data → transform → call an LLM → store results)
- Add reliability features (retries, logging, consistent outputs)
...
Before deploying, test your Flow to catch configuration and payload issues early.

- **Test**: run the entire Flow with sample input
- **Debug**: inspect node-level outputs and logs when something fails

See: [Flow Editor](/docs/flows/editor) and [Testing](/docs/tests)
...
- **My changes aren’t reflected**: Deploy your project so the latest Flow is active.
- **A node fails**: Open logs, confirm required credentials/inputs are set, and re-run the test with a small payload.
- **Payload missing fields**: Verify field names/types match what downstream nodes expect.

## Next steps

- Learn more about building blocks: [Nodes](/docs/nodes)
- Learn about deployment: [Deployments](/docs/deployments)