...
- **Status**: Indicates the status code of the execution (e.g., 200 for success).
  ...
  | 400 | Bad Request. Malformed request payload or invalid input. |
  | 401 | Unauthorized. Invalid or missing API key. |
  | 403 | Forbidden. Valid credentials, insufficient permissions. |
  | 404 | Not Found. Flow, node, or resource doesn't exist. |
  | 422 | Unprocessable Entity. Input didn't match expected schema/validation. |
  ...
  **Upstream/Provider Errors** — errors from the LLM provider, not your flow:

  | Status Code | Description |
  |-------------|--------------------------------------------------------------------------------------------------------------|
  | 502 | Bad Gateway. The model provider (OpenAI, Anthropic, etc.) itself errored. |
  | 503 | Service Unavailable. Provider overloaded/down, no fallback model available. |
  | 504 | Gateway Timeout. Model provider took too long to respond. |
