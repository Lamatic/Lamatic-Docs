...
For testing purposes, you need to manually provide `document_key`, `document_id`, `content`, and `document_url` as dummy data. Once deployed, these values will be automatically fetched from your configured Google Drive folder.
...
```json
{
    ...
    "document_url": "https://storage.googleapis.com/example-bucket/test_document.pdf?signature=..."
}
```

- **document_key**: The document's filename, used to locate or reference the specific file.
- **document_id**: The Google Drive file ID. Use this as the unique identifier for the document.
- **content**: The document's content or text data.
- **document_url**: A temporary, downloadable URL for the file's content.
...
- **document_key**: The document's filename, used to locate or reference the specific file.
- **document_id**: The Google Drive file ID. Use this as a unique identifier for the document, for deduplication, lookups, or referencing the file elsewhere in your flow.
- **content**: The document's content or text data.
- **document_url**: A temporary, downloadable URL for the file's content. Since it's short-lived, use it within the same flow run rather than storing it for later, pass it directly to a downstream node such as [Doc Extractor](/docs/nodes/ai/doc-extractor) or [Document Handling](/docs/nodes/data/document-handling-node).
