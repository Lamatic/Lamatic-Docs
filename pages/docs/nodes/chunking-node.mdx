...
1. **Flexible Integration:** Incorporate Lamatic.ai’s Chunking node into various workflows for tailored data processing solutions.
...
1. **Scalable Workflows:** Manage large datasets effectively, enabling workflows to scale seamlessly with increasing data.
...
1. Create workflows that handle large datasets by breaking them into manageable chunks.
...
- `_chunks`: An array of objects, each representing a segmented portion of the input text and its associated metadata.

- `pageContent`: The textual content of the chunk, extracted from the input data.

- `metadata`: A nested object providing additional details about the chunk’s origin or properties.

- `loc`: A nested object specifying the location of the chunk within the source document.

- `lines`: A nested object indicating the line range of the chunk in the source document.

- `from`: The starting line number of the chunk.

- `to`: The ending line number of the chunk.
