...
If the server is protected by IP-based network rules, review [IP Allowlisting](/docs/whitelist-ip) before connecting.
...
| **Problem** | **Solution** |
|-------------|---------------|
| **Connection failed** | Verify host, port, database, and network/firewall access. See [IP Allowlisting](/docs/whitelist-ip). |
