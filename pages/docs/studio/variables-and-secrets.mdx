...
The **Variables & Secrets** page allows users to securely store API keys, authentication tokens, and other sensitive data for seamless use across their workflows. By defining these secrets in the settings, you can reference them within any flow without exposing them directly.
...
- ⚡ **Efficiency:** Quickly integrate API keys and secrets into workflows without manual entry.
- 🔄 **Reusability:** Use stored secrets across multiple flows without reconfiguration.

## How to Add Secrets

1. Navigate to **Settings** > **Variables & Secrets**.
2. Click on **Add Secrets**.
3. Enter a **Secret Name** and its corresponding **Secret Value**.
4. Click **Create** to save the secret.

> Secrets are encrypted and stored securely, ensuring safe access in your workflows.
