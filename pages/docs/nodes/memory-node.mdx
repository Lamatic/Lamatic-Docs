---
title: Memory Node
description: The Memory Node enables storing and managing contextual information for users and sessions, allowing for persistent data storage within workflows
type: Data
order: 4
---

# Memory Node Documentation

The Memory Node allows users to store and manage contextual information by creating persistent memory collections tied to unique users and sessions. This node is particularly useful for maintaining conversation history, user preferences, and other stateful data across workflow executions.
...
| ------------------------- | ---------------------------------------------------------------------- | ------------------------ |
| **Unique Id**             | Identifier for the user whose memory is being stored                   | `1ab2`                   |
| **Session Id**            | Optional identifier for specific user sessions                         | `2 (Defaults to Global)` |
| **Memory Collection**     | The collection where memories will be stored                           | `MemoryStore`            |
