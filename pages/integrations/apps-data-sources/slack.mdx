...
<Callout type="info">
**Threads are not supported yet.** The Slack integration can only post messages to a channel. It cannot reply in a thread, and it cannot be triggered by a reply to a thread the bot created. Thread support is planned for a future release.
</Callout>
