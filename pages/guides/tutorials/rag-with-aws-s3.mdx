...
3. Choose **S3 Node** as the trigger.  
   Refer to [S3 Node setup instructions](https://lamatic.ai/docs/integrations/aws-s3).
...
### 3. Chunk Data for Processing

Chunking splits documents into smaller, meaningful pieces. This step ensures a balance between context and relevance for the chatbot.

1. Add a **Chunking Node**.
2. Configure chunking parameters:
   - Click the **+** icon to select the data to chunk.
   - Adjust settings under **Additional Properties**.
3. Use a **Logic Node** to extract chunked text:
   - Add the **Extract Chunked Text Node** and define transformation logic.

![Chunk Configuration](./img/rag-with-google-drive/chunk-config.png)
...
![Vectorization](./img/rag-with-google-drive/vactor.png)
...
1. Add a **Vector Node** for database storage.
2. Configure the node:
...
![RAG Configuration](./img/rag-with-google-drive/rag.png)
...
![Chatbot Integration](./img/building-chatbot/chatbot-integration.png)
