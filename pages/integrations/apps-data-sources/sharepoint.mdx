...
<Callout type="warning">
There is no exclusion syntax. A pattern starting with `!` (as in `.gitignore`) is matched literally and won't exclude anything. To leave a folder or file type out, list what you want instead of trying to subtract it. See [Glob Patterns](/docs/glob-patterns) for the full syntax reference.
</Callout>
