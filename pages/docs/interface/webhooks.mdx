---
title: Webhooks
description: Set up webhook triggers in Lamatic to run flows from external events with real-time HTTP callbacks.
---
...
<Callout type="info">
  If your webhook receiver is protected by a firewall or IP-based access policy,
  see [IP Allowlisting](/docs/ips) before enabling production traffic.
</Callout>
...
4. **Test the Webhook**  
   Send a sample payload or event to the endpoint to verify functionality. A webhook accepts any JSON body, whatever you send becomes the trigger node's output, available to downstream nodes as `{{triggerNode_1.output.<field>}}`.

   ```bash
   curl -X POST "<YOUR_WEBHOOK_ENDPOINT>" \
   ...
   With the payload above, `{{triggerNode_1.output.topic}}` resolves to `"hello world"` anywhere later in the flow.
