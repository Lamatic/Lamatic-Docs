---
title: For Loop Node
description: The For Loop Node enables iteration over a range of values or a list, allowing repeated execution of processes within a workflow
type: Data
order: 3
---

# For Loop Node Documentation

The For Loop Node allows users to execute loops in a workflow by iterating over a specified range or a list of objects. This node is particularly useful for automating repetitive tasks, handling batch processing, and managing multiple data points within a single workflow execution.

![forloop.png](./img/forloop/forloop.png)
...
| **Increment**     | (For Range) Defines the step size for each iteration.   | `1000`                          |
